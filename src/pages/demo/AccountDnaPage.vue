...
<template>
  ...
  <section v-else-if="apiMode" class="dna-page" aria-labelledby="api-dna-title">
    ...
    <div v-else class="dna-page__grid">
      ...
      <DnaEvidenceCard title="Recurring Phrases" :status="apiStatus(apiDna.recurringPhrases.length ? apiDna.recurringPhrases[0].status : 'INSUFFICIENT_DATA')"><blockquote v-for="item in apiDna.recurringPhrases" :key="item.quote">{{ item.quote }}</blockquote><p v-if="!apiDna.recurringPhrases.length">Not available</p></DnaEvidenceCard>
