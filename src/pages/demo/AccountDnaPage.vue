...
<template>
  ...
  <section v-else-if="apiMode" class="dna-page" aria-labelledby="api-dna-title">
    ...
    <div v-else class="dna-page__grid">
      ...
      <DnaEvidenceCard v-if="apiDna.valuePropositions.length" title="Value Propositions" :status="apiStatus(apiDna.valuePropositions[0].status)"><blockquote v-for="item in apiDna.valuePropositions" :key="item.quote">{{ item.quote }}</blockquote></DnaEvidenceCard><section v-else class="dna-page__empty">No value propositions available.</section>
      <DnaEvidenceCard title="Problem Framing" :status="apiStatus(apiDna.problemFraming.status)"><p>{{ apiDna.problemFraming.description ?? 'Not available' }}</p><blockquote>{{ apiDna.problemFraming.quote ?? 'Not available' }}</blockquote></DnaEvidenceCard>
      <DnaEvidenceCard title="Proof Style" :status="apiStatus(apiDna.proofStyle.status)"><dl class="dna-page__facts"><div><dt>PRIMARY</dt><dd>{{ apiDna.proofStyle.primary ?? '—' }}</dd></div><div><dt>SECONDARY</dt><dd>{{ apiDna.proofStyle.secondary ?? '—' }}</dd></div></dl><p>{{ apiDna.proofStyle.description ?? 'Not available' }}</p></DnaEvidenceCard>
      <DnaEvidenceCard title="CTA Patterns" :status="apiStatus(apiDna.ctaPatterns.status)"><p>{{ apiDna.ctaPatterns.style ?? 'Not available' }}</p><p>{{ apiDna.ctaPatterns.description ?? 'Not available' }}</p><div class="dna-page__tags"><span v-for="example in apiDna.ctaPatterns.examples" :key="example">{{ example }}</span></div></DnaEvidenceCard>
      <DnaEvidenceCard v-if="apiDna.recurringPhrases.length" title="Recurring Phrases" :status="apiStatus(apiDna.recurringPhrases[0].status)"><blockquote v-for="item in apiDna.recurringPhrases" :key="item.quote">{{ item.quote }}</blockquote></DnaEvidenceCard><section v-else class="dna-page__empty">No recurring phrases available.</section>
